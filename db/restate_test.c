...
static void test_collector_destructor(void* state) {
  free(state);
}
...
static bool test_collector_finish(
  ...
  rocksdb_user_collected_properties_insert(properties, "restate.test.entry_count",
                                           buf);
  return true;
}
...
int main(int argc, char** argv) {
  ...
  StartPhase("put_data");
  {
    ...
    for (i = 0; i < 50; i++) {
      ...
      rocksdb_put_cf(db, woptions, cf_handle, key, strlen(key), val, strlen(val),
                     &err);
      CheckNoError(err);
    }
  ...
  StartPhase("get_properties_of_all_tables");
  {
    ...
    uint64_t num_entries = rocksdb_table_properties_get_num_entries(table_props);
    ...
    size_t cf_name_len;
    const char* cf_name =
        rocksdb_table_properties_get_column_family_name(table_props, &cf_name_len);
    CheckCondition(cf_name != NULL);
    fprintf(stderr, "  Column family: %.*s\n", (int)cf_name_len, cf_name);

    // Test out of bounds access
    const char* null_name =
        rocksdb_table_properties_collection_file_name(props, count + 10, &name_len);
  ...
  StartPhase("get_properties_of_tables_by_level");
  {
    ...
    rocksdb_get_properties_of_tables_by_level(db, NULL,  // default column family
    ...
    for (level = 0; level < num_levels; level++) {
      size_t count = rocksdb_table_properties_collection_count(props_by_level[level]);
    ...
    // Clean up
    rocksdb_table_properties_collection_array_destroy(props_by_level, num_levels);
  }
  ...
  StartPhase("user_collected_properties");
  {
    ...
    // The existing rocksdb_table_properties_get_user_collected_property function
    // should work with the rocksdb_table_properties_t* returned from our
    // collection.
    ...
    size_t key_count = 0;
    const char** keys =
        rocksdb_table_properties_get_user_collected_property_keys(table_props,
                                                                   "", &key_count);
  ...
  StartPhase("user_collected_properties_with_collector");
  {
    ...
    rocksdb_options_add_table_properties_collector_factory(
        opts_with_collector,
        NULL,  // state
        collector_factory_destructor,
        collector_factory_name,
